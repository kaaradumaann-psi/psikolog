...
export function BeckAnxietyPage() {
  ...
  const liveResult = useMemo(() => {
    return calculateBeckAnxiety(answers, {
  ...
  function handleSave() {
    ...
    const result = calculateBeckAnxiety(answers, {
      clientId: selectedClientId || undefined,
    ...
    <div className="clinical-container">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16, marginBottom: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
            <div>
              ...
              <strong style={{ fontSize: 16, color: 'var(--text)' }}>{liveResult.subjectiveScore}</strong>
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--muted)' }}>Nörovejetatif</div>
              <strong style={{ fontSize: 16, color: 'var(--text)' }}>{liveResult.neurovegetativeScore}</strong>
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--muted)' }}>Otonomik Uyarılma</div>
              <strong style={{ fontSize: 16, color: 'var(--text)' }}>{liveResult.autonomicScore}</strong>
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--muted)' }}>Motor Belirtiler</div>
              <strong style={{ fontSize: 16, color: 'var(--text)' }}>{liveResult.motorScore}</strong>
            </div>
          </div>
        </div>
      ...
      <div className="btn-print-hide">
        ...
        {BECK_ANXIETY_SYMPTOMS.map((sym, idx) => (
          <div
            key={sym.id}
            className={`question-item-card ${answers[idx] !== 0 ? 'answered' : ''}`}
          >
