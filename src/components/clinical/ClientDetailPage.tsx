...
type Tab = 'overview' | 'sessions' | 'tests' | 'progress' | 'reports';
...
export function ClientDetailPage({ clientId }: { clientId: string }) {
  ...
  function handleSaveSoap(e: React.FormEvent) {
    ...
    const sessionToSave: SoapSession = {
      ...
      date: soapForm.date || new Date().toISOString().split('T')[0]!,
    ...
    <div className="clinical-container">
      ...
      <div className="clinical-header">
        <div className="clinical-title-wrap">
          ...
          <p>{client.gender === 'ERKEK' ? 'Erkek' : 'Kadın'}, {client.age} Yaş · {client.occupation || 'Meslek Belirtilmedi'} · Tel: {client.phone || '—'}</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 16 }}>
          <div className="modern-table-card" style={{ padding: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 13 }}>
              ...
              <div><strong>TC Kimlik:</strong> {client.tcNumber || '—'}</div>
          ...
          <div className="modern-table-card" style={{ padding: 20, gridColumn: '1 / -1' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12, fontSize: 13 }}>
              ...
              <div>
                <strong>Tıbbi &amp; Psikiyatrik Geçmiş:</strong>
                <p style={{ margin: '4px 0 0', color: 'var(--soft)' }}>{client.psychiatricHistory || client.medicalHistory || 'Özellik yok'}</p>
              </div>
              <div>
                <strong>Kullandığı İlaçlar:</strong>
                <p style={{ margin: '4px 0 0', color: 'var(--soft)' }}>{client.medications || 'Yok'}</p>
              </div>
            </div>
          </div>
        </div>
