  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div className="field">
          <label className="field-label">Harici Kaynak (mmpi/other)</label>
          <input className="input" {...register('externalSource')} placeholder="mmpi" maxLength={32} />
          <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
            MMPI puanlama bu sistemde yapılmaz — sadece harici kaynak olarak ilişkilendirilir (source="mmpi")
          </div>
        </div>
        <div className="field">
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <div className="field">
        ...
        <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 4 }}>
          Ham MMPI soruları/normları/scoring kopyalanamaz — sadece özet sonuç (örn. geçerlilik notu, yüksek ölçekler metinsel)
        </div>
      </div>
  ...
  if (items.length === 0) {
    return (
      <div className="empty-state-card">
        ...
        <p>MMPI harici kaynak olarak ilişkilendirilebilir — puanlama bu sistemde yapılmaz</p>
      </div>
    );
  }
